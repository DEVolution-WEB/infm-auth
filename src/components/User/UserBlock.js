	...
	return (
		<div className='w-full flex-grow sm:w-80 md:w-80 mb-10 p-12 shadow-md rounded'>
			...
			<div className='w-full'>
				<Autocomplete fullWidth='true' disablePortal onChange={(event, value) => setQuery(value?.user_name)} id='combo-box-demo' options={selectedUsers} renderInput={(params) => <TextField {...params} label='Sélectionner utilisateur' variant='standard' color='warning' />} getOptionLabel={(option) => `${option?.user_name} `} />
			</div>
