				...
				<div className='mb-6 py-15 px-10 md:px-0'>
					<div className='mb-10 p-12 shadow-md rounded'>
						...
						<div className='w-64'>
							...
							<select className='w-64 block bg-gray-100 text-black opacity-80 border-b border-white py-3 px-4 mb-3 mt-1 leading-tight focus:outline-none focus:border-yellowCust' name='user' onChange={(e) => setQuery(e.target.value)}>
								<option></option>
				...
				.map(({ id, data: { user_name, email, formation_id, created_date } }, index) => (
					<div key={index}>
						...
						<div className='bg-white py-5 px-10 md:px-0 mb-6'>
							<div>
								<div className='flex mb-5 space-x-2'>
									<h1 className='text-xl text-black'>Nom:</h1>
									<p className='text-lg text-black'>{user_name}</p>
								</div>
								<div className='flex mb-5 space-x-2'>
								...
								<div className='flex mb-5 space-x-2'>
									...
									<p className='text-lg text-black'>{formation_id}</p>
								</div>
								<div className='flex mb-5 space-x-2'>
									<h1 className='text-xl text-black'>Date de création:</h1>
									<p className='text-lg text-black'>{created_date}</p>
								</div>
								<div class='flex px-8 md:justify-center md:px-0'>
									...
									<button onClick={() => setModal(!modal)} className='bg-darkBlueCust mt-3 py-2 px-4 text-sm text-white rounded border border-blue-light focus:outline-none focus:border-primary hover:bg-blue-800 transition hover:duration-300'>
										Supprimer
									</button>
								</div>
							</div>
						...
						<div className='bg-grey-light' key={index}>
							{edit && <Edituser edit={edit} setEdit={setEdit} handleEditUsers={handleEditUsers} id={id} setUserName={setUserName} setEmail={setEmail} setFormationId={setFormationId} />}
						</div>
					</div>
				))}
		</div>
